...
parse: (core_query | error)* EOF;

// NOTE: RuntimeError changed to JavaScript's Error
error:
    UNEXPECTED_CHAR {
     throw new Error("UNEXPECTED_CHAR=" + $UNEXPECTED_CHAR.text);
   };

core_query:
    K_SELECT selection_strategy? list_of_variables from_clause K_WHERE cel_formula (
...
from_clause: ( K_FROM stream_name ( COMMA stream_name)*)?;
...
partition_list:
    LEFT_SQUARE_BRACKET attribute_list RIGHT_SQUARE_BRACKET (
        COMMA LEFT_SQUARE_BRACKET attribute_list RIGHT_SQUARE_BRACKET
    )?;

attribute_list: attribute_name ( COMMA attribute_name)*;
...
predicate:
    ...
    | math_expr K_IN K_RANGE LEFT_PARENTHESIS math_expr COMMA math_expr RIGHT_PARENTHESIS #
        in_range_predicate;

string_literal: string | attribute_name;

string_literal_or_regexp: string | attribute_name | regexp;
...
string_seq: string (COMMA string)*;

time_window: event_span | time_span | custom_span;

event_span: integer K_EVENTS;

time_span: hour_span? minute_span? second_span?;

hour_span: number K_HOURS;

minute_span: number K_MINUTES;

second_span: number K_SECONDS;

custom_span:
    integer LEFT_SQUARE_BRACKET any_name RIGHT_SQUARE_BRACKET;

named_event: s_event_name ( K_AS event_name)?;

s_event_name: ( stream_name '>')? event_name;
...
// regexp parser
regexp: '<<' regexp_alternation '>>';

regexp_alternation: regexp_exp ( '|' regexp_exp)*;

regexp_exp: regexp_element+;

regexp_element: regexp_group quantifier?;

regexp_group: parenthesis | atom;

parenthesis: REGEX_L_PAR regexp_alternation REGEX_R_PAR;

quantifier:
    REGEX_QUESTION
    | REGEX_PLUS
    | REGEX_STAR
    | REGEX_L_CURLY quantity REGEX_R_CURLY;
...
ccEscapes:
    '\\' (REGEX_HAT | REGEX_HYPHEN | REGEX_R_BRACK | '\\');
